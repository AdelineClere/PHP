...
/*
⚠️ PDO = classe PHP prédéfinie qui permet de se connecter à une BDD
Cette class possède ses propres propriétés et méthodes = ⚠️ class déjà instanciée
*/
echo '<pre>'; var_dump($pdo); echo '</pre>';
// $pdo représente un OBJET issu de la classe PDO permettant d'être connecté à la BDD 
...
*/
        // en comm sinon rajoute à chaq rafraichissement...!
...
// $resultat = on créé une ⚠️ variable de réception
...
//  > query a mis résultat dans un objet (PDOStatement) ! > on peut pas aller piocher les infos > ut une fct pr
                /*  object(PDOStatement)#2 (1) {
                        ["queryString"]=>
                        string(46) "SELECT * FROM employes WHERE id_employes = 699"
                    }   */
/*
Lorsqu'on exécute une requête de sélection via la METHODE query() sur l'objet PDO:
Succès : on obtient un autre objet issu d'une autre class : PDOStatement.
         Cet objet a dc des méthodes et propriétés différentes !!
Echec  : boolean FALSE
$resulat est inexploitable en l'état, nous devons lui associer une METHODE fetch 
...
$employe = $resultat->fetch(PDO::FETCH_ASSOC);  // pour retourner sous un tablo ARRAY
// METHODE FETCH va retourner résultat en ARRAY
...
// $employe = $resultat->fetch(PDO::FETCH_BOTH); // index à la fois numériqt + le nom des champs
// echo'<pre>'; print_r($employe); echo '</pre>';
                /*   Array
                    (
                        [id_employes] => 699
...
// $employe = $resultat->fetch(PDO::FETCH_OBJ);    // retourne un OBJET ac le nom des champs
...
$resultat = $pdo->query("SELECT * FROM employes"); // ⚠️ on pioche ds class PDO ac la méthode query
    ...
    // ⚠️ rowCount = METHODE issue de class PDOStatement pr comtper nb lg retournées par la requête de sel° (=19)

while($contenu = $resultat->fetch(PDO::FETCH_ASSOC))  // > donne que le 1er employé si pas de boucle !
// Pr chaq tour de boucle while, la var $contenu retourne un ARRAY par employé, 
...
// Votre requête sort ^plusieurs résultats ? : une bopucle !!
// Votre requête ne doit sortir qu'un seul et unique résultat ? : pas de boucle
// Votre requête ne sort qu'un seul résultat et peut potentiellement en sortir plusieurs ? : une boucle 
