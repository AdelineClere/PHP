...
// ⚠️  Si on clique sur le lien deconnexion, on supprime la session (cf. header '?action' lg 65)
{
    session_destroy();
}
            ...
            if($membre['mdp'] == $_POST['mdp']) // => ⚠️  on contrôle si mdp saisi = mdp BDD
            {
                foreach($membre as $indice => $valeur)  // ⚠️  On passe en revue infos du membre qui a bon pseudo + bon mdp
                {
                    if($indice != 'mdp')    // ⚠️  On exclu le mdp qui n'est pas conservé ds le fichier session
                    {
                        ...
                        // ⚠️  On créé ds le fichier session un tablo membre et on enregistre les données de l'internaute qui pourra dès à présent naviguer sur le site sans être connecté.
                    }
                }
                // debug($_SESSION);
